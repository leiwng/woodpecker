                ...
                for cur_kyt_chromo in cur_kyt_chromos:
                    ...
                    if best_shape_match_ai_chromo is not None and best_shape_match_ai_chromo["id"] == cur_chromo_id:
                        ...
                        try:
                            ...
                            if upside_down:
                                # 染色体颠倒
                                log.info(
                                    f"AI{best_shape_match_ai_chromo['id']}号-{best_shape_match_ai_chromo['pos']} 染色体颠倒。"
                                )
                                new_err = {
                                    "标本编号": case_id,
                                    "图号": img_id,
                                    "错误说明": f"AI{best_shape_match_ai_chromo['id']}号-{best_shape_match_ai_chromo['pos']} 染色体颠倒。",
                                    "轮廓差异度": f"{shape_diff_score:.2f}%",
                                    "特征点相似度": f"{sift_sim_score:.2f}%",
                                    "错误个数": 1,
                                    "错误类型": "极性",
                                    "正确答案染色体编号": cur_kyt_chromo["id"],
                                    "正确答案染色体位置": cur_kyt_chromo["pos"],
                                    "AI识别出染色体编号": best_shape_match_ai_chromo["id"],
                                    "AI识别出染色体位置": best_shape_match_ai_chromo["pos"],
                                }
                                err_df = pd.concat(
                    ...
                    try:
                        ...
                        if best_sift_match_ai_chromo is not None and best_sift_match_ai_chromo["id"] == cur_chromo_id:
                            ...
                            if upside_down:
                                log.info(
                                    f"AI{best_sift_match_ai_chromo['id']}号-{best_sift_match_ai_chromo['pos']} 染色体颠倒。"
                                )
                                new_err = {
                                    "标本编号": case_id,
                                    "图号": img_id,
                                    "错误说明": f"AI{best_sift_match_ai_chromo['id']}号-{best_sift_match_ai_chromo['pos']} 染色体颠倒。",
                                    "轮廓差异度": f"{shape_diff_score:.2f}%",
                                    "特征点相似度": f"{sift_sim_score:.2f}%",
                                    "错误个数": 1,
                                    "错误类型": "极性",
                                    "正确答案染色体编号": cur_kyt_chromo["id"],
                                    "正确答案染色体位置": cur_kyt_chromo["pos"],
                                    "AI识别出染色体编号": best_sift_match_ai_chromo["id"],
                                    "AI识别出染色体位置": best_sift_match_ai_chromo["pos"],
                                }
                                err_df = pd.concat(
                                    [err_df, pd.DataFrame([new_err])],
                                    ignore_index=True,
                            ...
                            # 如果SIFT方法匹配的最佳匹配染色体同kyt染色体编号不一致(不再使用CLAHE增强后的SIFT方法匹配)，那么就认为AI推理结果错误
                            log.info(
                                f"{cur_kyt_chromo['id']}号-{cur_kyt_chromo['pos']} 染色体被AI识别为{best_sift_match_ai_chromo['id']}号-{best_sift_match_ai_chromo['pos']} 染色体。"
                            )
                            new_err = {
                                "标本编号": case_id,
                                "图号": img_id,
                                "错误说明": f"{cur_kyt_chromo['id']}号-{cur_kyt_chromo['pos']} 染色体被AI识别为{best_sift_match_ai_chromo['id']}号-{best_sift_match_ai_chromo['pos']} 染色体。",
                                "轮廓差异度": f"{shape_diff_score:.2f}%",
                                "特征点相似度": f"{sift_sim_score:.2f}%",
                                "错误个数": 1,
                                "错误类型": "识别",
